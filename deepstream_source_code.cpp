...
// for backup and work pgie with sgie also [LP]
...
//                    qDebug()<<"j pgie user meta type : "<<user_meta->base_meta.meta_type<<" "<<NVDS_USER_FRAME_META_EXAMPLE;

////                    if(user_meta->base_meta.meta_type == NVDS_USER_FRAME_META_EXAMPLE){

//                        DsExampleOutput* dsexample_user_meta_main = (DsExampleOutput*)user_meta->user_meta_data;
////                        metadata_main = dsexample_user_meta_main->matrix;
//                        //                            DsExampleObject* user_obj_meta = dsexample_user_meta_main->object;
//                        //                            qDebug()<<"user_obj_meta : "<<user_obj_meta->label; /// sabbir changed

//                        qDebug()<<"j object_id : "<<obj_meta->object_id<<" sndclass_id: "<<obj_meta->class_id<<" obj_label : "<<obj_meta->obj_label<<endl;


//                        metadata_main = (cv::Mat*) user_meta->user_meta_data;// main object person/car

//                        qDebug()<<"j metadata_one->rows : "<<metadata_main->rows<<" metadata_one->cols : "<<metadata_main->cols;

//                        std::string path = "/home/sigmind/Desktop/car/" ;
//                        if(metadata_main->rows > 0 && metadata_main->cols > 0)
//                            cv::imwrite(path+std::to_string(frame_meta->frame_num) + ".jpeg" ,*metadata_main);
...
// for deploy face and LP together
...
GstPadProbeReturn Deepstream_source_code::dsexample_buf_probe(GstPad *pad, GstPadProbeInfo *info, gpointer u_data)
{
    ...
    for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
            ...
            else if (obj_meta->class_id == 1) {///LP
