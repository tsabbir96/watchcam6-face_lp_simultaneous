...
//std::string passwd.c_str() = "00125680";
...
std::string get_rtsp::generate_rtsp(char szHostName[128],std::string passwd)
{
    ...
    PullPointSubscriptionBindingProxy proxyEvent;
      ...
      //  strcat(szHostName, "192.168.0.210");
...
//    }
    ...
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyDevice.soap, NULL, "admin", passwd.c_str()))
    {
        // return -1;
    }
    ...
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyMedia.soap, NULL, "admin", passwd.c_str()))
    {
        // return -1;
    }
    ...
    if (SOAP_OK == proxyMedia.GetProfiles(trt__GetProfiles, trt__GetProfilesResponse))
    {
        ...
        for (int i = 0; i < trt__GetProfilesResponse->Profiles.size(); i++)
        {
            ...
            if (SOAP_OK == proxyMedia.GetStreamUri(trt__GetStreamUri, trt__GetStreamUriResponse))
            {
                ...
                //ui->rtsp_label->setText(trt__GetStreamUriResponse->MediaUri->Uri.c_str());

                RTSPurl = trt__GetStreamUriResponse->MediaUri->Uri;
    ...
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyEvent.soap, NULL, "admin", passwd.c_str()))
    {
        // return -1;
    }
